...
public class BasicMovement {
	...
	public BasicMovement(AdvancedRobot _robot) {
		this.robot = _robot;
	}
	...
	public void onScannedRobot(ScannedRobotEvent e) {
		...
		Point2D enemyLocation = TargetingUtils.project(robotLocation, enemyAbsoluteBearing, enemyDistance);
		Point2D robotDestination;
		double tries = 0;
		while (!fieldRectangle.contains(robotDestination = TargetingUtils.project(enemyLocation, enemyAbsoluteBearing + Math.PI + direction,
				enemyDistance * (DEFAULT_EVASION - tries / 100.0))) && tries < MAX_TRIES) {
			tries++;
		}
		if ((Math.random() < (TargetingUtils.bulletTravelPerTick(enemyFirePower) / REVERSE_TUNER) / enemyDistance ||
				tries > (enemyDistance / TargetingUtils.bulletTravelPerTick(enemyFirePower) / WALL_BOUNCE_TUNER))) {
			direction = -direction;
		}
		// Jamougha's cool way
		double angle = TargetingUtils.absoluteBearing(robotLocation, robotDestination) - robot.getHeadingRadians();
